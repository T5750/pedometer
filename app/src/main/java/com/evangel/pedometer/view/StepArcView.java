...
public class StepArcView extends View {
	...
	private void drawArcYellow(Canvas canvas, RectF rectF) {
		...
		/**
		 * 绘制圆弧的方法 drawArc(RectF oval, float startAngle, float sweepAngle,
		 * boolean useCenter, Paint paint)//画弧，
		 * 参数一是RectF对象，一个矩形区域椭圆形的界限用于定义在形状、大小、电弧， 参数二是起始角(度)在电弧的开始，圆弧起始角度，单位为度。
	...
	private void drawTextNumber(Canvas canvas, float centerX) {
		...
		vTextPaint.getTextBounds(stepNumber, 0, stepNumber.length(),
				bounds_Number);
		canvas.drawText(stepNumber, centerX,
	...
	private void drawTextStepString(Canvas canvas, float centerX) {
		...
		canvas.drawText(stepString, centerX, getHeight() / 2 + bounds.height()
				+ getFontHeight(numberTextSize), vTextPaint);
	}
	...
	public void setCurrentCount(int totalStepNum, int currentCounts) {
		...
		/** 上次所走步数占用总共步数的百分比 */
		float scalePrevious = (float) Integer.valueOf(stepNumber)
				/ totalStepNum;
		/** 换算成弧度最后要到达的角度的长度-->弧长 */
	...
	/**
	 * 为进度设置动画 ValueAnimator是整个属性动画机制当中最核心的一个类，属性动画的运行机制是通过不断地对值进行操作来实现的，
	 * 而初始值和结束值之间的动画过渡就是由ValueAnimator这个类来负责计算的。 它的内部使用一种时间循环的机制来计算值与值之间的动画过渡，
		...
		progressAnimator
				.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
					...
					public void onAnimationUpdate(ValueAnimator animation) {
						...
						currentAngleLength = (float) animation
								.getAnimatedValue();
						invalidate();
					}
				});
