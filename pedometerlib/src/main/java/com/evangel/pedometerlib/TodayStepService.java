...
import com.evangel.pedometerlib.util.LibGlobals;
...
public class TodayStepService extends Service implements Handler.Callback {
	 ...
	 * 数据库中保存多少天的运动数据
	 */
	private static final int DB_LIMIT = 366;
	/**
	 * 保存数据库频率
	 */
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		// } else {
		// Toast.makeText(getApplicationContext(), "Lib 当前手机使用计步传感器",
		// Toast.LENGTH_LONG).show();
		//
		// }
		// }
	...
	private void initNotification(int currentStep) {
		...
		int smallIcon = getResources().getIdentifier("icon_step_small",
		...
		int largeIcon = getResources().getIdentifier("icon_step_large",
				"mipmap", getPackageName());
		if (0 != largeIcon) {
			Logger.e(TAG, "largeIcon");
			builder.setLargeIcon(
					BitmapFactory.decodeResource(getResources(), largeIcon));
		} else {
	...
	private void addStepCounterListener() {
		...
		stepCounter = new TodayStepCounter(getApplicationContext(),
				mOnStepCounterListener, mSeparate, mBoot);
		Logger.e(TAG, "countSensor");
		sensorManager.registerListener(stepCounter, countSensor,
				SAMPLING_PERIOD_US);
	}
	...
	private void saveStep(int currentStep) {
		...
		sHandler.sendEmptyMessageDelayed(HANDLER_WHAT_SAVE_STEP,
				LAST_SAVE_STEP_DURATION);
	...
	private void cleanDb() {
		...
		mDbSaveCount = 0;
		if (null != mTodayStepDBHelper) {
			mTodayStepDBHelper.clearCapacity(DateUtils.dateFormat(
					System.currentTimeMillis(), "yyyy-MM-dd"), DB_LIMIT);
		}
		// if (null != mTodayStepDBHelper) {
		// 保存多天的步数
	...
	private final ISportStepInterface.Stub mIBinder = new ISportStepInterface.Stub() {
		...
		private JSONArray getSportStepJsonArray(
				List<TodayStepData> todayStepDataArrayList) {
			return SportStepJsonUtils
					.getSportStepJsonArray(todayStepDataArrayList);
		}

		@Override
		public String getTodaySportStepArray() throws RemoteException {
			if (null != mTodayStepDBHelper) {
				List<TodayStepData> todayStepDataArrayList = mTodayStepDBHelper
						.getQueryAll();
				JSONArray jsonArray = getSportStepJsonArray(
						todayStepDataArrayList);
		...
		@Override
		public String getTodaySportStepArrayByDate(String date)
				throws RemoteException {
			if (null != mTodayStepDBHelper) {
				List<TodayStepData> todayStepDataArrayList = mTodayStepDBHelper
						.getStepListByDate(date);
				JSONArray jsonArray = getSportStepJsonArray(
						todayStepDataArrayList);
				Logger.e(TAG, jsonArray.toString());
				return jsonArray.toString();
		...
		@Override
		public String getTodaySportStepArrayByStartDateAndDays(String date,
				int days) throws RemoteException {
			if (null != mTodayStepDBHelper) {
				List<TodayStepData> todayStepDataArrayList = mTodayStepDBHelper
						.getStepListByStartDateAndDays(date, days);
				JSONArray jsonArray = getSportStepJsonArray(
						todayStepDataArrayList);
				Logger.e(TAG, jsonArray.toString());
				return jsonArray.toString();
		...
		try {
			...
			if (null != activityInfos && activityInfos.length > 0) {
				for (int i = 0; i < activityInfos.length; i++) {
					...
					Class superClazz = Class.forName(receiverName)
							.getSuperclass();
					int count = 1;
					while (null != superClazz) {
						...
						if (superClazz.getName()
								.equals(BaseClickBroadcast.class.getName())) {
	...
	private void microlog4AndroidError(String msg) {
		if (null != mMicrolog4Android) {
			mMicrolog4Android
					.error(DateUtils.getCurrentDate("yyyy-MM-dd HH:mm:ss")
							+ "   " + msg);
		}
	}
}
