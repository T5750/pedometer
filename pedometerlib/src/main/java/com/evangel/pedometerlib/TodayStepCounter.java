...
/**
 * android4.4 Sensor.TYPE_STEP_COUNTER 计步传感器计算当天步数，不需要后台
 */
class TodayStepCounter implements SensorEventListener {
	...
	public TodayStepCounter(Context context,
			OnStepCounterListener onStepCounterListener, boolean separate,
			boolean boot) {
	...
	public void onSensorChanged(SensorEvent event) {
		if (event.sensor.getType() == Sensor.TYPE_STEP_COUNTER) {
			...
			PreferencesHelper.setElapsedRealtime(mContext,
					SystemClock.elapsedRealtime());
			PreferencesHelper.setLastSensorStep(mContext, counterStep);
			Logger.e(TAG,
					"counterStep : " + counterStep + " --- " + "sOffsetStep : "
	...
	private boolean shutdownBySystemRunningTime() {
		if (PreferencesHelper.getElapsedRealtime(mContext) > SystemClock
				.elapsedRealtime()) {
