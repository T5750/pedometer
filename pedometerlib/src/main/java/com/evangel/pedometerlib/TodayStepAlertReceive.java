...
public class TodayStepAlertReceive extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		if (ACTION_STEP_ALERT.equals(intent.getAction())) {
			boolean separate = intent.getBooleanExtra(
					TodayStepService.INTENT_NAME_0_SEPARATE, false);
			Intent stepInent = new Intent(context, TodayStepService.class);
			stepInent.putExtra(TodayStepService.INTENT_NAME_0_SEPARATE,
					separate);
			context.startService(stepInent);
			StepAlertManagerUtils
					.set0SeparateAlertManager(context.getApplicationContext());
