...
/**
 * Sensor.TYPE_ACCELEROMETER 加速度传感器计算当天步数，需要保持后台Service
 */
class TodayStepDetector implements SensorEventListener {
	...
	public TodayStepDetector(Context context,
			OnStepCounterListener onStepCounterListener) {
		super();
		mContext = context;
	...
	private synchronized void dateChangeCleanStep() {
		// 时间改变了清零，或者0点分隔回调
		if (!DateUtils.getTodayDate().equals(mTodayDate)) {
			WakeLockUtils.getLock(mContext);
			mCount = 0;
