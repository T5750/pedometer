...
class TodayStepDBHelper extends SQLiteOpenHelper implements ITodayStepDBHelper {
	...
	private static final String SQL_QUERY_STEP_BY_DATE = "SELECT * FROM "
			+ TABLE_NAME + " WHERE " + TODAY + " = ?";
	private static final String SQL_DELETE_TODAY = "DELETE FROM " + TABLE_NAME
			+ " WHERE " + TODAY + " = ?";
	// 只保留mLimit天的数据
	...
	public synchronized boolean isExist(TodayStepData todayStepData) {
		Cursor cursor = getReadableDatabase().rawQuery(SQL_QUERY_STEP,
				new String[] { todayStepData.getToday(),
	...
	public synchronized List<TodayStepData> getQueryAll() {
		Cursor cursor = getReadableDatabase().rawQuery(SQL_QUERY_ALL,
				new String[] {});
	 ...
	 */
	@Override
	public synchronized List<TodayStepData> getStepListByDate(
			String dateString) {
	...
	/**
	 * 根据时间和天数获取步数列表 例如： startDate = 2018-01-15 days = 3 获取
	 ...
	 */
	@Override
	public synchronized List<TodayStepData> getStepListByStartDateAndDays(
			String startDate, int days) {
		List<TodayStepData> todayStepDatas = new ArrayList<>();
		for (int i = 0; i < days; i++) {
			Calendar calendar = Calendar.getInstance();
			calendar.setTimeInMillis(DateUtils.getDateMillis(startDate,
					DATE_PATTERN_YYYY_MM_DD));
			calendar.add(Calendar.DAY_OF_YEAR, i);
			Cursor cursor = getReadableDatabase()
					.rawQuery(SQL_QUERY_STEP_BY_DATE,
							new String[] { DateUtils.dateFormat(
	...
	/**
	 * 根据limit来清除数据库 例如： curDate = 2018-01-10 limit=0;表示只保留2018-01-10 curDate =
	 * 2018-01-10 limit=1;表示保留2018-01-10、2018-01-09等
	 * 
	 * @param curDate
	 * @param limit
	 *            -1失效
	...
	public synchronized void clearCapacity(String curDate, int limit) {
		...
		try {
			...
			calendar.setTimeInMillis(
					DateUtils.getDateMillis(curDate, DATE_PATTERN_YYYY_MM_DD));
			calendar.add(Calendar.DAY_OF_YEAR, -(mLimit));
			String date = DateUtils.dateFormat(calendar.getTimeInMillis(),
					DATE_PATTERN_YYYY_MM_DD);
			Log.e(TAG, date);
			...
			for (TodayStepData tmpTodayStepData : todayStepDataList) {
				long dbTodayDate = DateUtils.getDateMillis(
						tmpTodayStepData.getToday(), DATE_PATTERN_YYYY_MM_DD);
				if (calendar.getTimeInMillis() >= dbTodayDate) {
					delDateSet.add(tmpTodayStepData.getToday());
				}
			}
			for (String delDate : delDateSet) {
				getWritableDatabase().execSQL(SQL_DELETE_TODAY,
						new String[] { delDate });
			}
		} catch (Exception e) {
			e.printStackTrace();
