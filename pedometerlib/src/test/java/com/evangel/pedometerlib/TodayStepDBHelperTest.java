	...
	public void insert() {
		...
		for (int i = 0; i < 10; i++) {
			...
			calendar.setTimeInMillis(
					DateUtils.getDateMillis("2018-01-01", "yyyy-MM-dd"));
			if (4 == i || 5 == i || 7 == i || 8 == i) {
				continue;
			}
			...
			for (int j = 0; j < 5; j++) {
				...
				todayStepData.setToday(DateUtils
						.dateFormat(calendar.getTimeInMillis(), "yyyy-MM-dd"));
				todayStepData.setStep(100 * j);
				Calendar hourCalendar = Calendar.getInstance();
				hourCalendar.setTimeInMillis(DateUtils
						.getDateMillis("2018-01-01 08:00", "yyyy-MM-dd HH:mm"));
		...
		List<TodayStepData> todayStepDataList = mTodayStepDBHelper
				.getQueryAll();
		...
		System.out.println(jsonArray.toString());
		todayStepDataList = mTodayStepDBHelper
				.getStepListByStartDateAndDays("2018-01-03", 3);
		jsonArray = getSportStepJsonArray(todayStepDataList);
		System.out.println(jsonArray.toString());
	...
	private JSONArray getSportStepJsonArray(
		...
		for (int i = 0; i < todayStepDataArrayList.size(); i++) {
			...
			try {
				...
				subObject.put(TodayStepDBHelper.TODAY,
						todayStepData.getToday());
				subObject.put(SPORT_DATE, DateUtils.dateFormat(
						todayStepData.getDate(), "yyyy-MM-dd HH:mm"));
